...
\afigure{img/utf8-bpl.png}{Bytes per \lineitem~used by with and without UTF-8 string encoding, Benchmark \ref{bm:q1} with scaling factors SF0.01 and SF0.1.}{fig:utf8-bpl}{0.9}
\afigure{img/utf8-load.png}{Data source load time for full compression with and without UTF-8 string encoding, Benchmark \ref{bm:q1}, SF0.1.}{fig:utf8-load}{0.6}
...
\afigure{img/gap-statistics.png}{Using statistics to select the proper storage format. A statistics provider service will, for a given data descriptor, return relevant statistics, preferably the number of rows, number of unique values, and average data element size. The module might cache, restructure and calculate some of the statistics manually, as the different database providers have different interfaces.}{fig:gap-statistics}{0.8}
