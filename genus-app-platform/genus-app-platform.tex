...
\subsection{Analysis}
\label{sub:Analysis}
\afigure{img/gap-analysis.png}{\gap~analysis user interface component.}{fig:genus-discovery}{1.0}
...
The \cn{CompositionFieldDescriptor} class describes an attribute in a composition, which is the object class property we studied in Section \ref{sub:Object Class Properties}. It holds all relevant data about a property, mainly type and representation, but also constraints and formatting rules. Like \cn{CompositionDescriptor}, instances of the \cn{DataDescriptor} class exist in the ISD layer.
...
%«Commonly in Genus App Platform, there are operations that perform calculations or evaluate predicates on a large number of values.”: I Discovery-sammenheng er det riktig, men sjekk med f.eks. Einar om setningen holder vann generelt.
%3.5: JA; leselighet går alltid på bekostning av performance
%3.5.3; Uten at jeg er helt sikker; er ikke dette tilfelle med alle programmeringsspråk som har "boxed" typer eller rtti?
...
\afigure{img/gap-original-instantiation.png}{A data source with with composition objects. For each object, there is not only value pointers, but also pointers to the corresponding data descriptors.}{fig:gap-original-instantiation}{0.8}
...
\textbf{RQ1: How does storage techniques used in an in-memory, analytics-oriented database, like columns and compression, affect \gap's ability to handle large datasets, and how must the application be changed to accommodate and utilize the new storage format?} 
