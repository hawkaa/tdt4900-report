...
Memory usage is measured for various states in the program by querying the built-in \name{FastMM} memory manager, and is commonly used to measure deltas in memory allocation. To measure the entire application footprint, including the .NET layer and the GUI, the built-in memory manager for Windows 10 is used.
...
\end{itemize}
\afigure{img/write-test-definition.png}{The task definition for the Write Test. For every attribute, the data source is enumerated.}{fig:write-test-definition}{0.4}
